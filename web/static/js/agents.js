...
(function() {
    ...
    function renderSidebar() {
        ...
        if (agents.length === 0) {
            container.innerHTML = '<div class="text-gray-500 text-xs px-3 py-2">No agents found</div>';
            return;
        }
    ...
    function createModal() {
        ...
        modalEl.innerHTML = ''
            ...
            + '      Agents Manager'
            + '    </h2>'
            + '    <button onclick="ClawIDEAgents.closeManager()" class="p-1.5 text-gray-400 hover:text-white hover:bg-gray-800 rounded-lg transition-colors">'
            ...
            + '        <button onclick="ClawIDEAgents.newAgent()" class="w-full flex items-center justify-center gap-1.5 px-3 py-2 text-xs text-indigo-400 hover:text-white hover:bg-gray-800 rounded-lg transition-colors">'
            ...
            + '      <div class="flex-1 flex items-center justify-center text-gray-500 text-sm">Select an agent or create a new one</div>'
            + '    </div>'
            + '  </div>'
            + '</div>';
    ...
    function renderModalList() {
        ...
        if (filtered.length === 0) {
            container.innerHTML = '<div class="text-gray-500 text-xs text-center py-4">No agents found</div>';
            return;
        }
        ...
        for (var i = 0; i < filtered.length; i++) {
            ...
            html += '<div class="px-2.5 py-2 rounded-lg cursor-pointer transition-colors '
                ...
                + '  <span class="text-sm text-white font-medium truncate">' + escapeHTML(ag.name) + '</span>'
                + '  ' + badge
                + '</div>';
            if (desc) {
    ...
    function renderEditor(ag) {
        ...
        pane.innerHTML = ''
            ...
            + '    <h3 class="text-sm font-semibold text-white">' + (isCreating ? 'New Agent' : escapeHTML(ag.name)) + '</h3>'
            + '    ' + (ag.scope === 'global'
            ...
            + '  </div>'
            + '  <div class="flex items-center gap-2">'
            + '    <button onclick="ClawIDEAgents.saveCurrentAgent()" class="px-3 py-1.5 text-xs bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg transition-colors flex items-center gap-1">'
            + '      <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>'
            + '      Save'
            + '    </button>'
            + (isCreating ? '' : '<button onclick="ClawIDEAgents.moveCurrentAgent()" class="px-3 py-1.5 text-xs text-gray-400 hover:text-white hover:bg-gray-700 rounded-lg transition-colors flex items-center gap-1">'
                + '<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"/></svg>'
                + (ag.scope === 'global' ? 'Move to Project' : 'Move to Global')
                + '</button>')
            + (isCreating ? '' : '<button onclick="ClawIDEAgents.deleteCurrentAgent()" class="px-3 py-1.5 text-xs text-red-400 hover:text-white hover:bg-red-900/50 rounded-lg transition-colors flex items-center gap-1">'
            ...
            + '      </div>'
            + '    </div>'
            + '    <p class="text-[10px] text-gray-600 mt-1.5">Allowed Tools: comma-separated list of tools this agent can access (e.g., Read, Write, Bash, Grep, Glob, Edit)</p>'
            + '  </div>'
            ...
            + '    <p class="text-[10px] text-gray-600 mt-1">The system prompt and instructions this agent follows when spawned. Markdown format.</p>'
            + '  </div>'
    ...
    function deleteCurrentAgent() {
        ...
        .then(function() {
            ...
            var pane = document.getElementById('agents-editor-pane');
            if (pane) {
                pane.innerHTML = '<div class="flex-1 flex items-center justify-center text-gray-500 text-sm">Select an agent or create a new one</div>';
            }
            loadAgents();
        })
        .catch(function(err) {
